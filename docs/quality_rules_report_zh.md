...
适用表：`dwd_stock_financial_indicator`、`dwd_stock_income`、`dwd_stock_balance_sheet`、`dwd_stock_cashflow`。
...
`balance_sheet_assets_equation` 仅适用于资产负债表，`cashflow_operating_net_flow` 仅适用于现金流量表。
